...
- `LessonContentBlock { id, type, order, config, xpReward }` — mirrors `LessonBlockResponse`. `type` is `INFO | INTRODUCE_SIGN | SELECT_MEANING | SELECT_SIGN | CONTEXT_RESPONSE | MATCH | VISUAL_RECOGNITION | PERFORM_SIGN | SPELL_NAME` (backend `BlockType`, which also has `INVISIBLE_SIGNS` — not rendered by the app yet). `config` is a **raw JSON string** — it's the same JSON `ContentLoader` builds straight from the lesson's YAML (`signa-api/content/dto/yaml/LessonBlockDto.java`), so it keeps the YAML's own key casing instead of the API's usual snake_case, and the client interceptor never touches it (it's a string leaf, not a nested object). Parse it with `parseBlockConfig<T>(block)`.
...
- `PracticeMistake { lessonBlockId, type, misses }` — mirrors `PracticeMistakeResponse`. `type` is `BlockType`; keyed by `lessonBlockId` rather than a sign meaning because `MATCH`/`VISUAL_RECOGNITION` blocks don't have a single one.
  - `PERFORM_SIGN`: `{ signs: string[], threshold?: number }` — the learner performs each sign in front of the camera and the model recognises it **on the device, in real time**. `signs` is declared like the other sign-listing blocks (`concepts` in MATCH, `sign_sequence` in VISUAL_RECOGNITION) and every entry must be a label the model knows (see `SIGNS_MODEL.labels` in `@/features/ml`); `signa-api`'s `PerformSignValidator` rejects the content at load time otherwise, because a sign the model can't recognise is an exercise nobody can pass. `reposo` is rejected too — it's the "not signing" class, not a sign. See [../features/ml.md](../features/ml.md).
  - `SPELL_NAME`: `{ max_letters?: number }` — the learner types their name and then fingerspells it letter by letter; the static alphabet model decides per frame instead of over a window. No sign list: the letters come from what the person typed, and `max_letters` (1-12, capped by `SpellNameValidator`) only bounds how long the exercise can get. See [../features/ml.md](../features/ml.md).
