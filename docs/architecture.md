...
| `features/` | Product-domain modules (onboarding, courses, practice, social, store, ml) | [features/onboarding.md](./features/onboarding.md) · [features/social.md](./features/social.md) · [features/store.md](./features/store.md) |
...
- `app.json` — app name "Signa", bundle `com.signasource.signamobile`, plugins `expo-font` + `expo-camera` + `expo-secure-store` + `expo-build-properties` + `expo-asset` + `expo-iap` (adds `com.android.vending.BILLING`), camera permission strings for both
  platforms, `userInterfaceStyle: light`. Native modules mean the app runs as a **development build** (`npx expo run:android`, EAS `development` profile with `expo-dev-client`), not in Expo Go; `android/` and `ios/` are generated by prebuild and git-ignored.
