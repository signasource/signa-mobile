...
- **Interceptor ↔ context bridge**: `setOnRefreshFailure` (from `src/api/client.ts`) is called on mount with a callback that calls `expireSession()`. This ensures that when the Axios 401 interceptor exhausts the refresh token, the session-expired modal shows up without needing to navigate manually.
